...
password = "<password>Dgo"  # Your Neo4j password
...
def get_lockdown_policy_influence(driver):
    ...
    MATCH (s)-[:TRIGGERED]->(b:PublicBehavior {type: 'Public Compliance'})
...
if len(G.nodes) == 0:
    ...
    # Display the graph
    plt.title("Media Influence on Lockdown Policies leading to Compliance Public Behavior", fontsize=15)
    plt.axis('off')
    plt.show()
